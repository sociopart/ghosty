...
export function DropzoneEditForm({
  handleFiles,
  initialImages,
  handleDeleteImage,
  initialImageIds,
    ...
    if (initialImages && initialImages.length > 0) {
      setPreviewFiles((prevFiles) => [
        ...
        ...initialImages.map((url, index) => ({
          id: `existing-${initialImageIds[index]}`,
          previewUrl: url,
        })),
      ]);
    }
  ...
  const handleHideImage = (fileId) => {
    setPreviewFiles((prevFiles) =>
      prevFiles.map((prevFile) => {
        if (prevFile.id === fileId) {
          const idOnly = prevFile.id.replace(/^existing-/, '');
          handleDeleteImage(idOnly);
          return { ...prevFile, hidden: true };
        }
        return prevFile;
      })
    );
  ...
  const preview = previewFiles.map((previewFile) => {
    if (previewFile.hidden) {
      return null; // Hide the image if hidden is true
    }

    return (
      <div key={previewFile.id} className="mr-4 text-base ml-5 w-1/8 h-1/8">
        <div className="relative">
          ...
          <button
            ...
            onClick={() => handleHideImage(previewFile.id)}
          >
            <svg
